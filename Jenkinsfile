                ...
                script {
                    dir('microservice') {
                        ...
                        */
                        ...
                        sh 'aws eks update-kubeconfig --name $EKSCLUSTERNAME --region $AWSREGION --kubeconfig .kube/config'
                        sh 'kubectl apply -f /front-end/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./catalogue-db/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./catalogue/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./carts-db/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./carts/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./queue-master/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        ...
                        sh 'kubectl apply -f ./user/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./orders-db/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./orders/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./payment/manifests -n $NAMESPACE --kubeconfig .kube/config'
                        sh 'kubectl apply -f ./shipping/manifests -n $NAMESPACE --kubeconfig .kube/config'
